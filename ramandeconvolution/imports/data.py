...
class DATA:
    ...
    def fitBaseline(self, degree, limits, **kwargs):
        # Select the part without Raman peaks and fit a poly func
        self.limits = limits
        baselineX = np.append(self.X[:np.argwhere(
                                    self.X>self.limits.min)[0][0]],
                                    self.X[np.argwhere(
                                    self.X>self.limits.max)[0][0]:])
        baselineY = np.append(self.current[:np.argwhere(
                                    self.X>self.limits.min)[0][0]],
                                    self.current[np.argwhere(
                                    self.X>self.limits.max)[0][0]:])
    ...
    def plotBaseline(self):
        ...
        if len(self.spikes):
                ax.plot(self.X[self.spikes], self.Y[self.spikes],
                                            'ro', label='Spikes')
        ax.plot(self.X, self.baseline, 'r--', label = 'Baseline')
        ax.plot([self.limits.min, self.limits.min],
                                [min(self.Y), max(self.Y)], 'r-')
        ax.plot([self.limits.max,self.limits.max],[min(self.Y),
                     max(self.Y)], 'r-', label='Excluded region')
    ...
    def detectSpikes(self, threshold):
        ...
        for i in np.arange(0, len(self.current)-2):
            ...
            if abs(previous-current)/current>threshold:
                self.spikes= np.append(self.spikes,
                                           [i, i+1, i+2]).astype(int)
